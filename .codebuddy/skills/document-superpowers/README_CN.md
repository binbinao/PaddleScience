...
### 上下文与调研
- **reference-material** - 读取并提取 `docs/reference/` 中的参考资料（支持 txt、markdown、pdf、图片）
- **research-gathering** - 在规划和执行之间，按章节搜索和整理参考资料（文章、图片），为写作提供素材支撑（可选阶段）

### 进阶
- **subagent-driven-writing** - 每节使用独立子代理快速迭代，配合两阶段审阅

### IT 技术文档增强插件
- **tech-doc-writing** - 技术文档编排插件：注入技术专属头脑风暴问题、提供 8 种文档类型模板、添加技术准确性审阅
- **tech-doc-code-blocks** - 代码块生成插件：渐进式复杂度（最简 → 生产 → 高级）、语法验证、格式规范
- **tech-doc-mermaid** - Mermaid 图表生成插件：图表类型选择指南、语法最佳实践、文档类型到图表的映射
- **tech-doc-image-search** - 图片管理插件：图片占位符管理、Alt Text 生成、来源推荐、视觉密度指南
...
>>>>>>> origin/feature/reference-material
```
...
安装后在 Codebuddy 的配置中添加技能路径引用即可使用。
...
├── YYYY-MM-DD-主题-research.md       # 参考资料（可选）
├── assets/
│   └── YYYY-MM-DD-主题/             # 下载的图片（可选）
├── drafts/
│   └── YYYY-MM-DD-主题/
...
│   ├── README.md                           # 目录说明
│   ├── *.txt, *.md, *.pdf                  # 文本类参考资料
│   └── *.png, *.jpg, *.gif, *.webp        # 图片类参考资料
└── writing/                                # 写作产物目录（输出）
    ├── YYYY-MM-DD-主题-references.md       # 参考资料摘要（如使用参考资料）
    ├── YYYY-MM-DD-主题-brief.md            # 已批准的主题简报
    ├── YYYY-MM-DD-主题-plan.md             # 详细大纲
    ├── drafts/
    │   └── YYYY-MM-DD-主题/
...
与其他技能配合使用：
- **参考材料** - 将背景文档放入 `docs/reference/`，自动提取上下文
- **调研技能** - 在规划阶段收集论据
- **SEO 技能** - 在审阅阶段进行优化
- **发布技能** - 定稿后进行部署
