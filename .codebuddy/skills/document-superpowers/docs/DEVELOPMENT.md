...
| 阶段 | 技能模块 | 路径 | 职责 |
|------|---------|------|------|
| 0. 参考材料 | `reference-material` | `skills/reference-material/SKILL.md` | 读取 docs/reference/ 中的参考文件（txt/md/pdf/图片），提取上下文 |
...
以下 4 个插件以增强层形式嵌入核心工作流，在检测到技术文档写作意图时自动激活：

| 插件 | 路径 | 增强阶段 | 职责 |
|------|------|---------|------|
| `tech-doc-writing` | `skills/tech-doc-writing/SKILL.md` | 头脑风暴 + 规划 + 审阅 | 编排器：注入技术问题、提供 8 种文档类型模板、添加第 5 轮技术准确性审阅 |
| `tech-doc-code-blocks` | `skills/tech-doc-code-blocks/SKILL.md` | 执行 | 代码块生成：语言标签、渐进式复杂度、语法验证、格式规范 |
| `tech-doc-mermaid` | `skills/tech-doc-mermaid/SKILL.md` | 执行 | Mermaid 图表：类型选择指南、语法模板、复杂度控制 |
| `tech-doc-image-search` | `skills/tech-doc-image-search/SKILL.md` | 执行 | 图片管理：占位符标准、Alt Text 规范、来源推荐、视觉密度 |

**插件激活机制：** 当 brainstorming 阶段的 brief 包含 `doc_type` 字段时，`tech-doc-writing` 自动在后续阶段加载对应模板并触发其他三个插件。

**支持的 8 种文档类型：**
1. Tutorial / How-To Guide（教程/指南）
2. Architecture Design Document（架构设计文档）
3. API Reference（API 参考文档）
4. Troubleshooting / Operations Manual（故障排查/运维手册）
5. Technical Concept Explainer（技术概念解析）
6. Technology Comparison（技术选型对比）
7. Best Practice / Style Guide（最佳实践/规范指南）
8. Release Note / Changelog（版本发布说明）

### 2.3 技能调用链

```
SKILL.md（入口）
  ├─→ reference-material/SKILL.md（如有参考材料）
  └─→ brainstorming/SKILL.md
  │     ↑ tech-doc-writing 注入技术专属问题（当检测到技术文档意图时）
  │
  └─→ writing-article-plan/SKILL.md
  │     ↑ tech-doc-writing 提供文档类型模板（8 种预定义模板）
  │
  │   ├─→ executing-article-plan/SKILL.md（批量模式）
  │   └─→ subagent-driven-writing/SKILL.md（子代理模式）
  │         ↑ tech-doc-code-blocks 生成代码块
  │         ↑ tech-doc-mermaid 生成 Mermaid 图表
  │         ↑ tech-doc-image-search 管理图片占位符
  │
  └─→ reviewing-article/SKILL.md
        ↑ tech-doc-writing 添加第 5 轮技术准确性审阅
```
...
├── docs/
│   └── reference/                         # 参考材料目录
│       └── README.md                      # 目录说明（支持格式、组织方式）
├── examples/
│   └── typescript-blog-post.md            # 完整的四阶段写作示例
└── skills/
    ├── reference-material/
    │   └── SKILL.md                       # 预处理：参考材料读取与提取
    ├── brainstorming/
    ...
    ├── tech-doc-writing/
    │   └── SKILL.md                       # 技术文档编排插件（8 种文档类型模板）
    ├── tech-doc-code-blocks/
    │   └── SKILL.md                       # 代码块生成与验证插件
    ├── tech-doc-mermaid/
    │   └── SKILL.md                       # Mermaid 图表生成插件
    └── tech-doc-image-search/
        └── SKILL.md                       # 图片占位符与视觉资产管理插件
```
...
| Pass 5 | 技术准确性 (Technical Accuracy) | 代码可运行？版本正确？术语准确？（仅技术文档） |

问题按严重程度分级：**Critical**（阻断发布）→ **Moderate**（影响质量）→ **Minor**（可选修复）。

Pass 5 由 `tech-doc-writing` 插件注入，仅在技术文档写作时激活。
...
| 阶段产出 | 文件路径 |
|----------|---------|
| 参考摘要 | `docs/writing/YYYY-MM-DD-<topic>-references.md` |
...
- [x] 文档类型模板（8 种 IT 技术文档类型模板 — 通过 tech-doc-writing 插件实现）
- [ ] 组织风格指南集成（AP / 企业特定风格）
- [ ] SEO 优化审阅轮次
- [ ] 导出格式化器（Medium / WordPress / 多种 Markdown 变体）
- [ ] 社交媒体摘要生成器
- [x] 参考材料读取技能（reference-material）
- [ ] 研究技能集成
- [ ] 多作者协作模式
